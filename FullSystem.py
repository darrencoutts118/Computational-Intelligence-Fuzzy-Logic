...
def processRules(Rules, FuzzyValues, Curves):
    ...
    for i in range(len(Curves[outputKey])):
        c = Curves[outputKey][i];
    ...
    for i in range(len(Curves[outputKey])):
        if(Curves[outputKey][i].name in curvesToCheck):
            curve =Curves[outputKey][i]
            base = abs((curve.b + curve.beta) - (curve.a - curve.alpha));
            total = 0.5 * base * finalMappings[curve.name];

            centre = (curve.a - curve.alpha) + base/2 + offset;

            topnumbers.append(centre*total);
            bottomnumbers.append(total);

            defuzzyValues[curve.name] = total;
            centres[curve.name] = centre;

    answer = sum(topnumbers) / sum(bottomnumbers);
